...
######################################################### 
    ...
    def search_f3(self): #search breadth_first
        if self.isEmpty():
            return
        my = MyQueue()
        my.EnQueue(self.root)
        ...
        while not my.isEmpty():
            ...
            #Sửa điều kiện theo yêu cầu đề bài
                count += 1
            if count == 1:
            # Đề yêu cầu là node thứ mấy thì coun   t bằng bấy nhiêu (ví dụ đề yêu cầu 'first node' thì count = 1
                return p
            if p.left!=None:
                my.EnQueue(p.left)
            if p.right!=None:
    ...
    def post_order_search(self,node):
        ...
        while stack2:
            p = stack2.pop()
            if p.left is not None and p.right is not None and p.data.Age%2!=0: #sửa theo đề bài
                del_val = p
                count += 1
